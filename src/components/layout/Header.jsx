...
export default function Header() {
  ...
  const { unreadCount, notifications, markNotificationRead, markAllRead, darkMode, setDarkMode, getMyCourses } = useApp();
  ...
  return (
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 gap-4 flex-shrink-0 z-30">
      {/* Search */}
          ...
          {showNotifs && (
            <div className="absolute right-0 top-full mt-2 w-96 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden">
              ...
              <div className="max-h-96 overflow-y-auto scrollbar-thin">
                {recent5.length === 0 ? (
                  ...
                  <button
                    ...
                    className={`w-full flex gap-3 px-4 py-3 hover:bg-gray-50 text-left transition-colors ${!n.isRead ? 'bg-blue-50/50' : ''}`}
                  >
              ...
              <div className="border-t border-gray-100 p-2">
                <button onClick={() => { navigate('/notifications'); setShowNotifs(false); }} className="w-full text-center text-sm text-blue-600 hover:underline py-1">
                  Ver todas
                </button>
              </div>
            </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              <div className="text-sm font-medium text-gray-800 leading-tight">{user?.name?.split(' ')[0]}</div>
              <div className="text-xs text-gray-400 capitalize">{user?.role === 'professor' ? 'Docente' : user?.role === 'admin' ? 'Admin' : 'Estudiante'}</div>
            </div>
          </button>
          {showUser && (
            <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden fade-in">
