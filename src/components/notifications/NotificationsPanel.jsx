...
const ICONS = {
  announcement: { icon: MessageSquare, color: 'bg-orange-100 text-orange-600' },
  grade: { icon: Star, color: 'bg-amber-100 text-amber-600' },
  deadline: { icon: Clock, color: 'bg-red-100 text-red-600' },
  submission: { icon: BookOpen, color: 'bg-orange-100 text-orange-600' },
};
  ...
  return (
    <div className="max-w-2xl mx-auto fade-in">
      <div className="flex items-center justify-between mb-6">
        <h1 data-testid="notifications-title" className="text-2xl font-bold text-gray-900">Notificaciones</h1>
        <button onClick={markAllRead} data-testid="notifications-mark-all" className="flex items-center gap-2 text-sm text-orange-600 hover:underline">
        ...
        {notifications.length === 0 ? (
          ...
          return (
            <div key={n.id}
              ...
              className={`flex items-start gap-4 p-4 rounded-2xl border cursor-pointer transition-all hover:shadow-sm ${n.isRead ? 'bg-white border-gray-100' : 'bg-orange-50/50 border-blue-100'}`}>
